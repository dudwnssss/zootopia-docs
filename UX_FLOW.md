...
핵심 루프를 왼쪽→오른쪽으로 반영:
```
홈(현황+퀘스트) → 스카우트(보상 사용) → 도감(결과 확인) → 랭킹(동기부여) → 더보기(트레이드·프로필·알림장)
```

- 퀘스트는 홈 화면에 통합 (별도 탭 불필요)
- 더보기(`/more`): 트레이드, 프로필, 알림장
...
### 모바일 하단 탭바 구성 (TeacherNav)
홈 🏠 → 미션 🎯 → 알림장 📝 → 반 관리 🏫 → 더보기 ⋯

- **승인**은 미션 탭에 통합 (미션 페이지 상단에 "승인 대기 N건" 섹션 표시)
- **더보기** (`/teacher/more`): 승인 관리, QR코드, 자리배치, 월드미션, 가이드
